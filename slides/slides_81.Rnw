...
\subtitle{Week 8: P-values and hypothesis testing}
\date{Winter 2026}
...
knitr::opts_chunk$set(concordance = TRUE, fig.path = "figs81/",
...
# Make modelsummary output LaTeX via kableExtra (avoids tabularray/tinytable)
...
\begin{itemize}
\item Housekeeping.
\item Today: interpreting p-values; how hypothesis tests relate to confidence intervals; why \textit{statistical significance} is a shaky decision rule.
\end{itemize}
\end{frame}
...
modelsummary(list(lm1, lm2, lm3, lm4, lm5), stars = TRUE,
             ...
             gof_omit = 'Std|R2',
             output = 'figs81/card-krueger-table4.tex')
...
\let\endcenter\relax
\centering
\resizebox{.9\textwidth}{!}{\input{figs81/card-krueger-table4.tex}}
\end{table}
...
\nocite{amrhein2019retire}
\nocite{gerbermalhotra2008}
...
\scriptsize
<<>>=
t(apply(outmat, 2, quantile, probs = c(0.025, 0.975)))
@
\pause
\normalsize

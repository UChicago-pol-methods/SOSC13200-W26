...
<<setup_coin, include=FALSE>>=
X <- c(0, 1, 2)
probs <- c(1/16, 3/8, 9/16)
Ex <- sum(X * probs)
sdx <- sqrt(sum((X - Ex)^2 * probs))
@
...
%-------------------------------------------------------------------------------%
\begin{frame}{Repeated samples and the sample mean}
...
g +
  ...
  annotate("segment", x = 0, xend = 1, y = 0.2, yend = 0.2,
           arrow = arrow(length = unit(0.25, "cm")), color = 'skyblue') +
  annotate("point", x = 0, y = 0.2, color = 'skyblue') + 
...
%-------------------------------------------------------------------------------%
\begin{frame}{Pager's published callback rates}

\begin{figure}
\centering
\includegraphics[width=\textwidth,height=0.65\textheight,keepaspectratio]{"../assets/pager_2003".png}
\end{figure}

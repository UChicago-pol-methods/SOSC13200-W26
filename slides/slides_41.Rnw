...
knitr::opts_chunk$set(concordance = TRUE, fig.path = "figs41/")
...
<<plot_basic, include = FALSE, echo = FALSE>>=
...
\scriptsize
<<explore = FALSE>>=
ggplot(dat[sample(1:nrow(dat), 1e3),], aes(x = year_of_birth, y = education)) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0.2) + # jittered points
...
\centering
\resizebox{.6\textwidth}{!}{
<<fig.fig.width = 5, fig.fig.height = 5, echo = FALSE>>=
ggplot(dat[sample(1:nrow(dat), 1e3),], aes(x = year_of_birth, y = education)) +
...
\centering
\resizebox{.6\textwidth}{!}{
<<fig.fig.width = 6, fig.fig.height = 6, echo = FALSE>>=
ggplot(dat_agg, aes(x = year_of_birth_adj, 
                    y = deviation, 
                    fill = as.factor(quarter_of_birth),
                    label = quarter_of_birth)) +

...
\item Homework assignments
  \begin{itemize}
    \item Hw 3 due Sunday \pause
    \item If you spend $>30$ minutes without making progress, go to Stack Overflow and pose a question.
  \end{itemize}
\end{itemize}
...
%-------------------------------------------------------------------------------%
\begin{frame}{Concepts, measures, and scores}

Measurement validity links ideas to data. A useful framework (Adcock \& Collier) is:

\begin{itemize}
\item Background concept \(\rightarrow\) systematized concept \(\rightarrow\) indicators \(\rightarrow\) scores
\end{itemize}

\pause
Probability models describe the process that generates \textit{scores}. Validity asks whether those scores meaningfully map back to the \textit{systematized concept}.
...
\end{itemize}
\item If we are describing events with counterfactuals, such as turning out to vote or being recorded as voting, it can be useful to describe those events as probabilistic.\pause
...
%-------------------------------------------------------------------------------%
\begin{frame}[fragile]{Sampling repeatedly}
\scriptsize

If we run the simulation many times, the empirical frequencies should be close to the probabilities we assigned.
...
\begin{frame}{Simulation check: observed proportions}
\small

The simulated proportions across the four outcomes look like this:

\begin{center}
\begin{tabular}{cccc}
HH & HT & TH & TT \\
\Sexpr{round(observed_props[['HH']], 3)} &
\Sexpr{round(observed_props[['HT']], 3)} &
\Sexpr{round(observed_props[['TH']], 3)} &
\Sexpr{round(observed_props[['TT']], 3)}
\end{tabular}
\end{center}

\pause

Those are all close to \(0.25\), which is what we expect under independent fair coin flips.
...
%-------------------------------------------------------------------------------%
\begin{frame}{Simulation check: independence}
\scriptsize

The simulated probabilities are:

\begin{align*}
\widehat{\textrm{P}}[A] &= \Sexpr{round(PA, 3)} \\
\widehat{\textrm{P}}[B] &= \Sexpr{round(PB, 3)} \\
\widehat{\textrm{P}}[AB] &= \Sexpr{round(PAB, 3)} \\
\widehat{\textrm{P}}[A]\widehat{\textrm{P}}[B] &= \Sexpr{round(PA * PB, 3)}
\end{align*}

\pause

These are very close, which is what we expect when the two events are independent.
...
%-------------------------------------------------------------------------------%
\begin{frame}{A case where \(A\) and \(B\) are not independent}
\small

\begin{itemize}
\item \(A\) is still the event that we get a head on the first coin flip: \(A = \{HT, HH\}\).
\item \(B\) is now the event that we get heads on both coin flips: \(B = \{HH\}\).
...
<<echo=FALSE, include=FALSE>>=
PB_dep <- mean(result_n == 'HH')
PAB_dep <- mean(result_n == 'HH')
@

%-------------------------------------------------------------------------------%
\begin{frame}{Simulation check: dependence}
\scriptsize
...
\begin{align*}
\widehat{\textrm{P}}[A] &= \Sexpr{round(PA, 3)} \\
\widehat{\textrm{P}}[B] &= \Sexpr{round(PB_dep, 3)} \\
\widehat{\textrm{P}}[AB] &= \Sexpr{round(PAB_dep, 3)} \\
\widehat{\textrm{P}}[A]\widehat{\textrm{P}}[B] &= \Sexpr{round(PA * PB_dep, 3)}
\end{align*}

\pause

These are not the same, so \(A\) and \(B\) are not independent.
...
Example: Suppose everyone in the University of Chicago community is asked to report whether they voted.

\begin{itemize}
\item We are concerned with \textit{false negatives}, when someone voted but reports that they did not.
\item And \textit{false positives}, when someone reports voting but did not.
\end{itemize}
...
A student reports that they voted. What is the probability that they actually voted?
...
\begin{itemize}
\item Event $A$: person actually voted
\item Event $B$: person reports voting
\end{itemize}
...
\begin{align*}
\textrm{P}[A|B] & = \frac{\textrm{P}[B|A]\textrm{P}[A]}{\textrm{P}[B]} \\
&= \frac{0.90 \times 0.60}{0.70} \\
&= \Sexpr{round(PA_if_B, 3)}
\end{align*}
...
``30\% of people recorded as voting are under 25''

\pause
Does this tell us that young people vote at high rates?

\pause
Some things we should think about:
\begin{itemize}
\item What percentage of the population is under 25?
\item Are voting records based on self-reports or administrative files, and does misreporting differ by age?
\end{itemize}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width = \textwidth]{../assets/npr.png} 
\end{figure}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width = \textwidth]{../assets/guardian.png} 
\end{figure}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width = \textwidth, height = 0.33\textheight, keepaspectratio]{../assets/johnson-et-al-PNAS.png} 
\end{figure}
\scriptsize
...
% -------------------------------------------------------------------------------%
\begin{frame}[fragile]{Simulating \(X\) in R}

\scriptsize

We can simulate the random variable \(X\), where \(X\) is the number of heads in two fair coin flips.
...
result_n_tab <- table(result_n)
result_n_props <- prop.table(result_n_tab)
@

\begin{frame}{Empirical distribution of \(X\)}
\small

In 1,000 simulated draws, the counts and proportions look like this:

\begin{center}
\begin{tabular}{cccc}
\(x\) & 0 & 1 & 2 \\
count & \Sexpr{result_n_tab[['0']]} & \Sexpr{result_n_tab[['1']]} & \Sexpr{result_n_tab[['2']]} \\
proportion & \Sexpr{round(result_n_props[['0']], 3)} & \Sexpr{round(result_n_props[['1']], 3)} & \Sexpr{round(result_n_props[['2']], 3)}
\end{tabular}
\end{center}

\pause

These are close to the theoretical probabilities \(0.25\), \(0.50\), and \(0.25\).
...
\begin{figure}
\centering
<<fig.width = 4.6, fig.height = 3.0, echo=FALSE>>=
plotdata <- data.frame(
  x = c(-1, 0, 1, 2),
  xend = c(0, 1, 2, 3),
  fx = c(0, 1/4, 1/2, 1/4),
...
\begin{figure}
\centering
<<fig.width = 4.6, fig.height = 2.6, echo=FALSE>>=
...
%-------------------------------------------------------------------------------%
\begin{frame}[fragile]{Marginal distributions of \(X\) and \(Y\)}
\tiny
<<fig.width = 5.2, fig.height = 2.8, echo=FALSE>>=
...
ggplot(plotdata, aes(x = x, y = fx)) +
  ...
  annotate("segment", x = 0.5, xend = 0.0, y = 0.5, yend = 0.5,
           arrow = arrow(length = unit(0.25, "cm")), color = 'skyblue') +
  annotate("segment", x = 0.5, xend = 1, y = 0.5, yend = 0.5,
           arrow = arrow(length = unit(0.25, "cm")), color = 'skyblue') +
  ...
  annotate("point", x = 0.5, y = 0.5, color = 'skyblue') +
  ggtitle('PMF of X as number of heads in 1 fair coin flip')
@
}
\end{figure}
...
ggplot(plotdata, aes(x = x, y = fx)) +
  ...
  geom_vline(xintercept = 0.5, lty = 'dashed', color = 'skyblue') +
  annotate(geom="text", x=0.5, y=0.75, label="E[X]", col = 'grey') +
  annotate("segment", x = 0.5, xend = 0.25, y = 0.5, yend = 0.5,
           arrow = arrow(length = unit(0.25, "cm")), color = 'skyblue') +
  annotate("segment", x = 0.5, xend = 0.75, y = 0.5, yend = 0.5,
           arrow = arrow(length = unit(0.25, "cm")), color = 'skyblue') +
...
\begin{minipage}[t][0.54\textheight][t]{\textwidth}
...
\centering
\resizebox{.48\textwidth}{!}{
<<fig.width = 5, fig.height = 5, echo=FALSE, warning=FALSE>>=

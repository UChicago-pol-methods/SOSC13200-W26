...
knitr::opts_chunk$set(concordance = TRUE, fig.path = "figs31/")
...
Suppose turnout is 60\% in this population: $\textrm{P}[A]$
<<>>=
PA <- 0.60
@
\pause

The overall rate of reported voting is 70\%: $\textrm{P}[B]$
<<>>=
PB <- 0.70
@

\pause
If you voted, you report voting 90\% of the time: $\textrm{P}[B|A]$
<<>>=
PB_if_A <- 0.90
@
...
\begin{figure}
\centering
<<fig.width = 5, fig.height =3.8, echo=FALSE>>=
plotdata <- data.frame(
  x = c(-1, 0, 1, 2),
  xend = c(0, 1, 2, 3),
  fx = c(0, 1/4, 1/2, 1/4),

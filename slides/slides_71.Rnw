...
knitr::opts_chunk$set(concordance = TRUE, fig.path = "figs71/")
...
options(modelsummary_factory_default = "kableExtra")
options(modelsummary_factory_latex = "kableExtra")
tab2 <- modelsummary(lm_list, stars = TRUE,
                     coef_map = 'treatment', 
                     add_rows = cbind(`Control mean` = 'Control mean', round(as.data.frame(lm_list2),3)),
                     # gof_omit = 'R2',
                     output = 'latex_tabular')
tab2 <- gsub("\\\\begin\\{table\\}\\s*", "", tab2)
tab2 <- gsub("\\\\centering\\s*", "", tab2)
tab2 <- gsub("\\\\end\\{table\\}\\s*", "", tab2)
cat(tab2, file = '../assets/banerjee-et-al-table2.tex')
@

\begin{center}
\scriptsize
\input{../assets/banerjee-et-al-table2.tex}
\end{center}
...
tab2_alt <- modelsummary(lm_list_alt, stars = TRUE,
                         coef_map = 'treatment', 
                         add_rows = cbind(`Control mean` = 'Control mean', round(as.data.frame(lm_list2),3)), 
                         # gof_omit = 'R2',
                         output = 'latex_tabular')
tab2_alt <- gsub("\\\\begin\\{table\\}\\s*", "", tab2_alt)
tab2_alt <- gsub("\\\\centering\\s*", "", tab2_alt)
tab2_alt <- gsub("\\\\end\\{table\\}\\s*", "", tab2_alt)
cat(tab2_alt, file = '../assets/banerjee-et-al-table2alt.tex')
@

\begin{center}
\scriptsize
\input{../assets/banerjee-et-al-table2alt.tex}
\end{center}

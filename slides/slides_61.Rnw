...
<<fig.width = 5, fig.height =3.5, echo=FALSE, warning=FALSE, message=FALSE>>=
...
fit_intercept <- lm(reply_atall ~ leg_party, data = df)
fit_means <- lm(reply_atall ~ leg_party - 1, data = df)
@

\begin{itemize}
\item With an intercept, the Democratic mean is the intercept and the Republican contrast is the slope:
$$
\hat{Y}_i = \Sexpr{round(coef(fit_intercept)[1], 3)} + \Sexpr{round(coef(fit_intercept)[2], 3)} W_i
$$
\item Without an intercept, each coefficient is a group mean:
$$
\hat{\mu}_D = \Sexpr{round(coef(fit_means)[1], 3)}, \qquad
\hat{\mu}_R = \Sexpr{round(coef(fit_means)[2], 3)}
$$
\item These two parameterizations imply the same fitted values.
\end{itemize}
